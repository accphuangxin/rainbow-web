 ...
 *
 */
package org.rainbow.web.action;
...
import org.rainbow.web.model.MMDataGrid;
...
public class DispatcherAction {
	...
	public RainbowContext execute(RainbowContext context,HttpServletRequest request){
		...
		try{
			...
			//context.addAttr("org", RainbowSession.getUserSession().getSessionData().get("orgCode"));//通过机构获取权限
	...
	public RainbowContext query(HttpServletRequest request){
		...
		try{
			...
			RainbowSession.web2Service(request);
			context.addAttr("authUser", RainbowSession.getAuthUser());
